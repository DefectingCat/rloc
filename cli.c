        ...
        if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
            ...
            while (token != NULL) {
                ...
                if (args->n_exclude_dirs == 0) {
                    ...
                    if (!new_dirs) {
                        for (int j = 0; j < args->n_exclude_dirs; j++)
                            free(args->exclude_dirs[j]);
...
void cli_print_help(const char *prog_name) {
    ...
    printf("  --max-file-size=MB  Skip files larger than N megabytes (default: 100)\n");
    printf("  --exclude-dir=DIR   Exclude directories (comma-separated)\n");
    printf("  --include-lang=LANG Include only specified languages (comma-separated)\n");
    printf("  --exclude-lang=LANG Exclude specified languages (comma-separated)\n");
    printf("  --include-ext=EXT   Include only specified extensions (comma-separated)\n");
